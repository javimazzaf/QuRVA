...
function [CLBP_S,CLBP_M,CLBP_C,CLBP_V,CLBP_SN,CLBP_T] = clbp(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.1
...
if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    ...
    CLBP_T=hist(CLBP_T(:),0:(bins-1));
    
    if (strcmp(mode,'nh'))
        CLBP_S=CLBP_S/sum(CLBP_S);
        CLBP_M=CLBP_M/sum(CLBP_M);
        CLBP_T=CLBP_T/sum(CLBP_T);
    end
else
    ...
    elseif ((bins-1)<=intmax('uint16'))
        ...
        CLBP_T=uint16(CLBP_T);
    else
        CLBP_S=uint32(CLBP_S);
        CLBP_M=uint32(CLBP_M);
        CLBP_T=uint32(CLBP_T);
    end
    ...
    CLBP_T = padImages(CLBP_T, size(image), rx, ry, dx, dy);
